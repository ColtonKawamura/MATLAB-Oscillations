function Sediment_Acoustics_3D_fixed_springs_full_dashpot_Cluster_v2(K, M, Bv, w_D, Nt, N, P, W, seed)
...
for nn = isort(1:iskip:end)
    if(~left_wall_list(nn))
        ...
        if length(unique(x_temp))>100
            ...
            X = tvec(i0:end);
            Y = x_temp(i0:end)-x0(nn);
            yu = max(Y(round(end/2):end));
            yl = min(Y(round(end/2):end));
            yr = (yu-yl);                               % Range of ‘y’
            yz = Y-yu+(yr/2);
            ...
            [s fval exitflag] = fminsearch(fcn, [yr/2;  offset_guess]);                    % Minimise Least-Squares
            ...
            figure(2),
            plot3((x_all(nn,:)-x_all(nn,1))/A,(y_all(nn,:)-y_all(nn,1))/A,(z_all(nn,:)-z_all(nn,1))/A,'.')
            xlabel('$\Delta x / A$','Interpreter','latex','FontSize',20)
            ylabel('$\Delta y / A$','Interpreter','latex','FontSize',20)
            zlabel('$\Delta z / A$','Interpreter','latex','FontSize',20)
            axis equal
            drawnow
            pause
